...
then
  ...
  sudo echo -e "$colorramp" > $DATA_DIR/colors.txt
  flags=$(jq -r '.monobandpsuedocolor.flags' <<<$geography)
  torender=${filename%.*}_color.tif
  sudo $DC_DIR/docker-compose exec -T pg_gdal sh -c "sudo gdaldem color-relief $flags $DATA_DIR/$filename $DATA_DIR/colors.txt $DATA_DIR/$torender"
